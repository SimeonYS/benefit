...
class BenefitPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('benefit.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `benefit`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM benefit WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO benefit (date, title, link, content)"
